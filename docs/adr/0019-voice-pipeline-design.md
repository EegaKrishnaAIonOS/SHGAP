...
- Trade-offs: this explicitly supersedes ADR-0006 — if Bhashini/AI4Bharat access becomes available and is still wanted for the final deliverable, the STT/TTS provider swap is a real (if contained) piece of follow-up work, not a config change, since Pipecat's per-provider services aren't drop-in interchangeable at the API-shape level. TTS language is fixed for a call's lifetime (chosen once, from the member's stored preference) — a member who switches languages mid-call will still be transcribed correctly (STT auto-detects), but the assistant keeps replying in the language the call started in. The frontend piece that actually sends `sessionId`/`accessToken`/`language` and speaks WebRTC in a browser doesn't exist yet — T10's deliverable is the backend pipeline only, per the sprint plan's own "voice pipeline service" wording; a future task must build the browser-side connect/mic/speaker UI against the `/api/offer` contract documented here.

## Update: Sarvam STT instead of Groq/Whisper

Switched STT from `GroqSTTService` (Whisper) to `SarvamSTTService` (`saaras:v3`) shortly after this ADR was first accepted, at the user's suggestion — Sarvam's models are trained specifically on Indian languages/accents and support Telugu/English code-switching, and `saaras:v3` streams over a WebSocket rather than Whisper's upload-then-transcribe HTTP call, which should mean lower turn-taking latency in a live call (not independently benchmarked here, but structurally true of streaming vs. batch transcription).

Verified with a real round trip, not just documentation: a Telugu sentence ("మామిడి ఊరగాయను నమోదు చేయండి, ధర నూట ఇరవై రూపాయలు" — "register mango pickle, price one hundred twenty rupees") synthesized via Sarvam TTS and fed back through Sarvam STT transcribed correctly, including converting the spoken number into "₹120" — a level of formatting polish worth noting since it wasn't specifically asked for. STT is left at the model's own auto-detect ("unknown") rather than pinned to the session's configured language, for the same code-switching reason as before. Groq is now used for the LLM only.
