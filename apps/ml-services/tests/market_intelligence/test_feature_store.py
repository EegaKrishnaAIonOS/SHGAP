...
def test_write_features_creates_all_parquet_files_and_a_manifest(tmp_path, monkeypatch):
    ...
    trends_df = pd.DataFrame({"keyword": ["SHG products India"], "interest": [42.0]})

    manifest = feature_store.write_features(sales_df, price_df, trends_df)
    ...
    assert (tmp_path / "trends_features.parquet").exists()
    assert (tmp_path / "manifest.json").exists()


def test_write_features_defaults_trends_features_to_empty_when_omitted(tmp_path, monkeypatch):
    monkeypatch.setattr(settings, "feature_store_dir", str(tmp_path))
    manifest = feature_store.write_features(pd.DataFrame({"a": [1]}), pd.DataFrame({"b": [2]}))
...
def test_read_features_returns_empty_frames_before_any_run(tmp_path, monkeypatch):
    monkeypatch.setattr(settings, "feature_store_dir", str(tmp_path))
    sales, price, trends = feature_store.read_features()
    assert sales.empty
    assert price.empty
    assert trends.empty


def test_read_features_reads_back_all_three_tables(tmp_path, monkeypatch):
    monkeypatch.setattr(settings, "feature_store_dir", str(tmp_path))
    trends_df = pd.DataFrame({"keyword": ["SHG products India"], "interest": [42.0]})
    feature_store.write_features(pd.DataFrame({"a": [1]}), pd.DataFrame({"b": [2]}), trends_df)

    sales, price, trends = feature_store.read_features()
    assert len(sales) == 1
    assert len(price) == 1
    assert trends.iloc[0]["interest"] == 42.0
